 ...
 * @since 1.0.0
 */
public class BaseBizImpl<T,M> implements IBaseBiz<T,M> {
    private Class<T> clazz;
    private Class<M> mClazz;

    private BaseFactory<T,M> factory;

    public BaseBizImpl() {
        ParameterizedType t = (ParameterizedType) this.getClass().getGenericSuperclass();
        //获取泛型参数的实际类型
        this.clazz = (Class<T>) t.getActualTypeArguments()[0];
        this.mClazz = (Class<M>) t.getActualTypeArguments()[0];
        factory = new BaseFactory(clazz,mClazz);
    }
    ...
    public void save(M bean) {
        //T tBean = F
        Field id =  ReflectUtils.getIdField(factory.voToTable(bean).getClass());
        Object value = null;
        try {
            value = id.get(factory.voToTable(bean));
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public void insert(M bean) {
        entityManager.persist(factory.voToTable(bean));
    }

    @Transactional(propagation = Propagation.REQUIRED)
    public void update(M bean) {
        T tBean = factory.voToTable(bean);
        entityManager.merge(tBean);
    }

    @Transactional(propagation = Propagation.SUPPORTS)
    public M findOne(Serializable id) {
        return factory.tableToVo(entityManager.find(clazz, id));
    }
    ...
    public List<M> findAll() {
        ...
        return factory.tablesToVos(beans);
    }
    ...
    public List<M> findAllDesc() {
        ...
        return factory.tablesToVos(findHql(hql,clazz));
    }
    ...
    public List<M> pageListDesc(int pageNum, int pageSize) {
        ...
        Query query = entityManager.createQuery(hql,clazz);
        limitDatas(pageNum,pageSize,query);
        return factory.tablesToVos(query.getResultList());
    }
    ...
    @Override
    @Transactional(propagation = Propagation.SUPPORTS)
    public List<M> pageListDesc(String fieldName, int pageNum, int pageSize) {
        String hql = "select t from " + clazz.getSimpleName() + " t order by t."+fieldName+" desc";
        Query query = entityManager.createQuery(hql,clazz);
        limitDatas(pageNum,pageSize,query);
        return factory.tablesToVos(query.getResultList());
     ...
     */
    @Transactional(propagation = Propagation.REQUIRED)
    public void batchUpdate(List<M> beans) {
        if (beans != null) {
            for (M bean : beans)
    ...
    public List<M> findEqualField(String field, Object value) {
        ...
        return factory.tablesToVos(findHql(hql,clazz,value));
    }
    ...
    public List<M> findBetweenField(String field, Object start, Object end) {
        ...
        return factory.tablesToVos(findHql(hql,clazz,start,end));
    }
